...
import { formatMoney, formatDate, currentMonthKey } from "../utils/formatters";
...
export async function expenseConversation(conversation: MyConversation, ctx: MyContext): Promise<void> {
  ...
  const summary = [
    ...
    uz.transaction.summaryType(uz.categoryType.EXPENSE),
  ...
  await ctx.reply(uz.transaction.expenseSaved, { reply_markup: mainMenuKeyboard });

  const month = currentMonthKey(date);
  const alert = await conversation.external(() =>
    BudgetService.checkAfterTransaction(auth.family.id, category.id, month),
  );
  if (alert) {
    const message =
      alert.percent >= 100
    ...
    // not just whoever happened to add this expense.
    await notifyFamily(ctx.api, auth.family.id, message);
  }
}
